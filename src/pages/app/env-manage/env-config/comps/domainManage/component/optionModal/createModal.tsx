...
import { getBaseinfo } from '@/services/app/env-overview';
...
const GroupModal: React.FC<IGroupModal> = ({
  ...
  const handleGetBaseInfo = async (env: any) => {
    try {
      const res = await getBaseinfo(env);

      if (res.success) {
        setIdent('-' + res?.data?.envIdent);
      }
    } catch (error) {}
  };
  const handleGetsegent = async (v?: any) => {
    try {
      ...
      const inNet = currentDomain?.inNet || domainfo?.inNet;
      if (res.success) {
              ...
              rules={[
                { required: true },
                {
                  validator: (rule, value, callback) => {
                    /* 小写英文字母或数字或者“-”中划线，其中中划线不能在开头 */
                    if (!/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value)) {
                      callback('只能由小写英文字母或数字或者“-”中划线组成，其中中划线不能在开头');
                    }
                    callback();
                  },
                },
